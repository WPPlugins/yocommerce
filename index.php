...
function yocommerce_show_buy_button($content) {
	...
	$content .= 'Cost is ' . $goods_cost . ' $ <br>'; 
        ...
        $content .= '<input type="submit" name="submit" id="submit" class="button button-primary" value="Buy"  />';
        $content .= '</a>';
	}
...
function yocommerce_get_requests() {
	...
	if (isset($_GET['yocommerce_payment']) && $_GET['yocommerce_payment'] == 'return') {
		...
		$check_this_order = $wpdb->get_results ("SELECT * FROM `wp_plugin_yocommerce_orders` WHERE `order_token`='" . $_SESSION['TOKEN'] . "';");
		
		if (count($check_this_order) == 0) {		
				
		$wpdb->insert("wp_plugin_yocommerce_orders", array(
